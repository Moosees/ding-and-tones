...
* reset displayedChord and displayedNote when fetching a scale (or song with scale)
* merge drum reducer into other reducers?

### chords slice

* switch from old reducer to slice and update all action dispatches
* reset chords when fetching or loading a scale (or song with scale)

### howls slice

* switch from old reducer to slice and update all action dispatches
* reload howls when fetching or loading a scale (or song with scale)
...
* switch from old reducer to slice and update all action dispatches
* standardize loadSong inputs and helper functions
* editSong = false only when loading from search?
* isOwner when fetching or loading song - move to refs?
* transfer logic from ui reducer - autoMove(order), currentDropdown, mutedBars, isEditingSong, isSongPlaying, multiSelect, currentBar, currentBeat, currentHand?, currentSound, hands/headers/countOpen
* Rework and simplify logic for setCurrentlyPlaying?
* add stop song playback extra reducer (changing audioSrc, replace all setIsSongPlaying false?) remove useSongIsPlaying?
* remove close dropdown logic from components where needed
* remove resetUiState in patternPlayer.js and add reset for song state after song finish playing
* setCurrentDropdown closing logic is in both state and components - remove from components

### song api

* change handling of songId in url when saving or deleting?
* remove isOwner and songId when deleting song
* firstLoad arg not needed for getSongById?
* add alert if scale fails to load with song? split song and scale fetches?

### ui slice

* switch from old reducer to slice and update all action dispatches
* move actions to other reducers where possible
* Clean up UI state and reduce number of actions (also for other reducers?) - better solution than validKeys?
...
* move privacyOpen from ui to user?
