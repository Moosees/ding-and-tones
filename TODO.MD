...
* Show label for tak BeatDropdown SoundItems, "name" prop for soundList?
* Fix bug causing howls to unload when swapping between scales that contain the same note
* Cleanup and re-create howls when changing audio source
* Make sure loading scales sets root info correctly in parseScaleData

#### Optional changes

* Store percussive sounds/notes and options in shared location? (howls.options?)
* Remove key from howls state when removing note from scale?
* Change setCurrentlyPlaying to use note instead of option?
* Merge setCurrentlyPlaying into updateSoundForBeat?
...
* Only start loading howls after UI finish loading?

#### Cleanup

* Make sure loading status doesn't get overwritten when creating howls - done?
