...
* Adding or removing notes from scale updates drum
* Adding or removing notes from scale updates keyboard listeners
* Adding or removing notes from scale updates BeatDropdown
* Adding or removing notes from scale updates pattern player
* Changing sound source does a full reset on drum click listeners
* Changing sound source does a full reset on keyboard listeners
* Changing sound source does a full reset on BeatDropdown click listeners
* Changing sound source does a full reset on pattern player
* Transposing scale does a iterative check on what to update
* Loading scale (or song with scale) does a iterative check on what to update

#### Needed changes

* Update play button to use new howls state structure
* Update keyboard to use new howls state structure
* Update howl loading status getting and setting
* Update howl playing components to check loading status before trying to play howl
* Make sure loading status doesn't get overwritten when creating howls
* Update cleanupHowls for new howls state structure (loading scale, getting scale etc.)
* Add percussive sounds to howls
* Add percussive keyboard handlers
* Add percussive hits to BeatDropdown
* Fix updateSoundForBeat to trigger animations on drum when clicking drum, playing song, by using keyboard or using beatDropdown
* Add howls handling when modifying scale
* Make sure loading scales sets root info correctly in parseScaleData

#### Optional changes

* Change setCurrentlyPlaying to use note instead of option?
* Howls reducer refactor to howls.utils?
* createFullScaleFromNames handles new createScaleLabel and returns label with scale?
* Add isOwner to song with scale fetch
* Memoize (scale)Notes to prevent rerender when changing between add round and add extra
* Add position to extra notes in scaleFull and use for displaying on drum and mini drum?
* Iterate over scale notes to see if song is ready to be played?
* Keep list of howl status for howls not currently in scale?
* Merge drum state into scale.ui?
* Does saving scale need to update scale state other than ID and isOwner?
* Use scale.parsed.pitched in MiniDrum, ExtraNote? (scale)Notes, Edit and Move?
* Handle checking if song is playing in state? Middleware?
* Update beat.utils to use parsed scale with (dynamic) percussive notes?
* Stop extra notes from updating positions when adding extra notes?

#### Cleanup

* Update flat/sharp to use split round and dings - done?
* Make sure chords work with new parsed scales - done?
* Refactor scale.utils (handle roots that are not ding, split file?)
* Rename/clean addRootAndPosition in scale.utils
* Refactor scale.actions?
* Remove old global effects - done?
* Remove utils for old global effects - done?
* Remove old state reducers and actions
* Remove logging and misc code
...
* Triad chords
* Remember sound source for user
* Set base rotation for scales to simulate drums that don't have lowest note closest to player
* Allow user to flip drum 180 deg (or freely?) to allow making tabs where high notes are in focus
